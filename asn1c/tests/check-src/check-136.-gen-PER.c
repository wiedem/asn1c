...
static unsigned char fbuf[0xffff];
static unsigned char fbuf2[0xffff];
...
load_object_from(const char *fname, unsigned char *fbuf, size_t size, enum enctype how, int mustfail) {
	...
	for(; csize < 20; csize += 1) {
		...
		ASN_DEBUG("LOADING OBJECT OF SIZE %zd FROM [%s] as %s,"
			" chunks %zd\n",
			...
			case AS_APER:
				...
				if(rval.code == RC_WMORE) {
					if(fbuf_chunk == fbuf_left) {
						ASN_DEBUG("-> PER decode error (%zd bits of %zd bytes (c=%d,l=%d)) \n", rval.consumed, size, fbuf_chunk, fbuf_left);
						rval.code = RC_FAIL;
						rval.consumed += 7;
						rval.consumed /= 8;
						if(mustfail) {
							ASN_DEBUG("-> (this was expected failure)\n");
							return 0;
						}
					} else {
		...
		if(how == AS_UPER) {
			ASN_DEBUG("[left %d, off %d, size %zd]\n",
...
compare_with_data_out(const char *fname, void *datap, size_t size, enum enctype how) {
	...
	sprintf(outName, SRCDIR_S "/data-136/%s", fname);
	if (how == AS_UPER) {
		strcpy(outName + strlen(outName) - 3, ".uper");
	} else if (how == AS_APER) {
		strcpy(outName + strlen(outName) - 3, ".aper");
	} else {
	...
	if((compare && !mustfail) && getenv("REGENERATE")) {
		...
		f = fopen(outName, "r");
		assert(f);
		rd = fread(fbuf2, 1, sizeof(fbuf2), f);
		assert(rd);
		fclose(f);

		fprintf(stderr, "Trying to decode [%s]\n", outName);
		load_object_from(outName, fbuf2, rd, how, mustfail);
		if(mustfail) return;

		if(compare) {
			assert(rd == (size_t)size);
			assert(memcmp(fbuf2, data, rd) == 0);
			fprintf(stderr, "XER->PER recoding .in->.out match.\n");
		} else {
			assert(rd != (size_t)size || memcmp(fbuf2, data, rd));
...
process_XER_data(const char *fname, unsigned char *fbuf, size_t size) {
	...
	/* Save and re-load as UPER */
	save_object_as(st, AS_UPER);
	compare_with_data_out(fname, buf, buf_offset, AS_UPER);
	st = load_object_from("buffer", buf, buf_offset, AS_UPER, 0);
	assert(st);
	...
	/* Save and re-load as APER */
	save_object_as(st, AS_APER);
	compare_with_data_out(fname, buf, buf_offset, AS_APER);
	st = load_object_from("buffer", buf, buf_offset, AS_APER, 0);
	assert(st);
...
process(const char *fname) {
	...
	snprintf((char *)fbuf, sizeof(fbuf), SRCDIR_S "/data-136/%s", fname);
...
main() {
	...
	str = getenv("DATA_136_FILE");
	if(str && strncmp(str, "data-136-", 9) == 0) {
		process(str);
		return 0;
	}

	dir = opendir(SRCDIR_S "/data-136");
	assert(dir);
	...
	while((dent = readdir(dir))) {
		if(strncmp(dent->d_name, "data-136-", 9) == 0)
