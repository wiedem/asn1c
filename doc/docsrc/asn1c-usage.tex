...
% Set asn1c version, as standalone build does not get it
\def\asnver{0.9.28u}

% Courier 10 Pitch
%\def\courierFont{Courier10 BT WGL4}
\def\courierFont{Courier New}
...
\usepackage[colorlinks=true,
	...
	citecolor={clrlink},
	urlcolor={clrurl},
	pdfauthor={Lev Walkin and Uri Blumenthal},
...
\author{Lev Walkin <\href{mailto:vlm@lionet.info?Subject=asn1c}{vlm@lionet.info}> 
	and Uri Blumenthal <\href{mailto:uri@mit.edu}{uri@mit.edu}>}
...
{\ttfamily -flink-skeletons} & {\small Symlink skeletons support files rather than copy them.}\\
{\ttfamily -fno-skeletons-copy} & {\small Do not copy skeletons support files.}\\\\
...
following example uses the definition of \emph{write\_stream()} from up there.
\begin{codesample}
/*
...
\subsection{\label{sub:Encoding-PER}Encoding PER}
The PER stands for Packet Encoding Rules, defined in ITU standard \url{http://www.itu.int/ITU-T/studygroups/com17/languages/X.691-0207.pdf}. 
The encoder routine API comes in three flavors: stdio-based, callback-based,
and using buffer (either pre-allocated, or new -- aka allocated by the encoder itself).
With the callback-based encoder, the encoding process is very similar
to the DER one, described in Section~\ref{sub:Encoding-DER}. With the stdio-based
encoder, the encoding process is similar to the XER one, 
described in Section~\ref{sub:Encoding-XER}.The
buffer-based example is 
provided here.
\begin{codesample}
\end{codesample}	

\subsection{\label{sub:Decoding-PER}Decoding PER}
